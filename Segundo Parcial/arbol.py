...
# FUNCIONES EJERCICIO 1
    def inorden_nombre (self):
        if (self.info is not None):
            if (self.izq is not None):
                self.izq.inorden_nombre()
            print ('El nombre del dinosaurio es: ', self.datos['nombre'], ' y su codigo es: ', self.datos['código'])
            if (self.der is not None):
                self.der.inorden_nombre()

    def mostrar_informacion (self, dino):
        pos = self.busqueda(dino)
        if (pos):
            print('Nombre: ', pos.datos['nombre'], ' Codigo: ', pos.datos['código'], ' ubicación: ', pos.datos['ubicación'])
        else:
            print('No hay ningun dinosaurio con ese nombre.')
    
    def informacion_raptores (self, raptor):
        pos = self.busqueda(raptor)
        if (pos):
            print('ubicación: ', pos.datos['ubicación'])
        else:
            print('No hay ningun raptor.')

    def modificar_nombre (self, dino):
        pos = self.busqueda(dino)
        if (pos):
            pos.datos['Sgimoloch'] = 'Stygimoloch'
            print('Nombre: ', pos.datos['nombre'], ' Codigo: ', pos.datos['código'], ' ubicación: ', pos.datos['ubicación'])
        else:
            print('No hay ninguna dinosaurio con el nombre Sgimoloch en el arbol.')

    def contar_nodos (self, Diplodocus):
        cantidad = 0
        if (self.info is not None):
            if (self.datos['nombre'] == Diplodocus):
                cantidad += 1
            if (self.izq is not None):
                cantidad += self.izq.contar_nodos(Diplodocus)
            if (self.der is not None):
                cantidad += self.der.contar_nodos(Diplodocus)
        return cantidad
